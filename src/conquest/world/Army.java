...
public class Army {
	...
	private Building dest; // destination building
	 ...
	 */
	public Army(Team t, Point start, Building dest, int nSoldiers, int attack, int speed) {
		this.team = t;
		// TODO support all building types (DONE)
		this.dest = dest;
		setAttack(attack);
	...
	/**
	 * update the army Each call to this methos is a processing cycle
	 */
	public void update() {
		// move the army
		pos.x += dir.x * speed;
		...
		if (dest.getDoorLocation().distance(pos) < speed) {
			// is dest is from the same team its a population change, otherwise its an
			// attack
	 ...
	 * attack a building
	 */
	private void attack(Building v) {
		// TODO support all building types (DONE)
