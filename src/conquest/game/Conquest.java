...
public class Conquest extends JFrame {
	...
	private BuildingSelection sel;  // currently selected towns 
	private Town dest;  	        // town selected as the destination
	...
	private void playLevel(){
		Team []es = { new Team("Romans", 12,9,3,40,0),
				      new Team("Neutral", 0,7,0,0,0),
				      new Team("Aztecs", 10,10,5,45,2),
				      new Team("Chinese", 9,12,4,35,4) };
		teams = es;
		playerTeam = teams[0];
	...
	/** 
	 * Method called automatically 30 times a second to update the game
	 * ATTENTION! This methods DOES NOT draw the game. Use method drawGame for that
	 */
	 ...
	 * This method only changes the game state,
	 * is does not deal with the game end neither displays any messages
	 */
	private void checkGameOver(){
		...
		else if( playerTeam.getTownCount() == 0 ){
			status = DEFEAT; 
		}
		else {
	...
	private void endLevel(){
		if( status == DEFEAT) {
			...
			int answer = JOptionPane.showOptionDialog(  null, "Crushing defeat! What do you want to do?", "DEFEAT", JOptionPane.YES_NO_OPTION,
			...
			int answer = JOptionPane.showOptionDialog(  null, "Amazing victory! What do you want to do?", "VICTORY", JOptionPane.YES_NO_OPTION,
	...
	private void ratoPremido( MouseEvent me ){
		...
		sel.addTown( world.getTownAt( end ) );
	}
	...
	private void ratoLibertado( MouseEvent me ){
		...
		if( dest != null ) {
			for( Town v : sel.getTowns() ) {
				// create an army in each selected town
				Army a;
				if( enlisting )
					a = v.enlistArmy( dest );
				else
	...
	/**
	 * Method that is used to draw the game elements
	 * ALL DRAWINGS MUST BE DONE IN THIS METHOD
	...
	private void drawMotionArrow( Graphics2D g ) {
		...
		// TODO support all building types
		// draw a line from the exit door of each town to the destination
		for( Town v : sel.getTowns() ) {
